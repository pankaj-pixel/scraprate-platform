...
const Page = path === '' ? App : path === '/admin/prices' ? AdminPrices : path === '/admin/import-prices' ? AdminImportPrices : path === '/admin/price-submissions' ? AdminPriceSubmissions : path === '/admin/data-sources' ? AdminDataSources : path === '/submit-price' ? SubmitPrice : path === '/scrap-prices' ? ScrapPricesPage : materialMatch ? () => <MaterialPricePage slug={materialMatch[1]} /> : publicPages.includes(path.slice(1)) ? () => <StaticPage page={path.slice(1)}/> : NotFoundPage;
