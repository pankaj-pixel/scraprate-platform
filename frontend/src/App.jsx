...
const FAQS = [
  ['How are ScrapRate prices calculated?', 'The current indicative rate combines eligible published observations using a consistent calculation. Real observations take priority over demo observations for each material, city and date.'],
...
function MaterialModal({ material, city, history, onClose }) {
  ...
  return <div className="modal-backdrop detail-backdrop" onClick={onClose}><div className="modal material-detail" role="dialog" aria-modal="true" aria-labelledby="modal-title" onClick={(e) => e.stopPropagation()}>
    ...
    <div className="detail-market-meta"><span className={`data-type-badge ${material.data_type}`}>{material.data_type === 'real' ? 'Published rate' : 'Demo data'}</span><span>{material.freshness_label}</span></div>
